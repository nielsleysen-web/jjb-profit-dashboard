...
import { syncNewMember, syncCancel } from "./klaviyo";
...
export async function createShopifyOrderForPaypal(sub, extra = {}) {
  ...
  await sendPurchase({
    ...
    contents: [{ id: META_CONTENT_ID, quantity: bundle.qty }],
  });

  // Abonnee → Klaviyo (lijst + event "Started Membership"); nooit blokkerend
  const trialEnds = sub.billing_info?.next_billing_time || new Date(new Date(sub.create_time || Date.now()).getTime() + MEMBERSHIP.trialDays * 86400000);
  await syncNewMember({
    email: s.email_address, firstName, lastName: rest.join(" "), phone,
    address: { address1: addr.address_line_1, city: addr.admin_area_2, zip: addr.postal_code, province: addr.admin_area_1 },
    provider: "paypal", subscriptionId: sub.id, qty: bundle.qty, bundleLabel: bundle.label, shippingTitle: ship.title,
    amountPaid: paid, trialEnds, orderName: created.name, membershipPrice: MEMBERSHIP.price / 100, intervalDays: MEMBERSHIP.intervalDays,
  });
  return created;
}
