...
/* ---------------- creatives: bestanden & thumbnails ---------------- */
// Map-ID uit een Drive-link halen (…/folders/<id> of ?id=<id>)
export function folderIdFromLink(link) {
  const m = String(link || "").match(/\/folders\/([A-Za-z0-9_-]{10,})|[?&]id=([A-Za-z0-9_-]{10,})/);
  return m ? m[1] || m[2] : "";
}

// Nieuwste video of afbeelding in een map waarvoor Drive al een thumbnail heeft.
export async function findCreativeFile(folderId) {
  const token = await getAccessToken();
  const q = [`'${esc(folderId)}' in parents`, "trashed = false", "(mimeType contains 'video/' or mimeType contains 'image/')"].join(" and ");
  const { data } = await axios.get(`${DRIVE_API}/files`, {
    params: { q, fields: "files(id, name, mimeType, hasThumbnail, createdTime)", orderBy: "createdTime desc", pageSize: 10, supportsAllDrives: true, includeItemsFromAllDrives: true },
    headers: { Authorization: `Bearer ${token}` },
    timeout: 15000,
  });
  const files = data.files || [];
  // liefst een bestand met thumbnail; video's krijgen die pas na verwerking door Drive
  return files.find((f) => f.hasThumbnail) || files[0] || null;
}

// Thumbnail-bytes ophalen via het service-account (de thumbnailLink zelf is niet publiek)
export async function fetchThumbnail(fileId, size = 640) {
  const token = await getAccessToken();
  const meta = await axios.get(`${DRIVE_API}/files/${encodeURIComponent(fileId)}`, {
    params: { fields: "thumbnailLink, mimeType, hasThumbnail", supportsAllDrives: true },
    headers: { Authorization: `Bearer ${token}` },
    timeout: 15000,
  });
  ...
  const img = await axios.get(sized, { responseType: "arraybuffer", headers: { Authorization: `Bearer ${token}` }, timeout: 15000 });
  return { buffer: Buffer.from(img.data), contentType: img.headers["content-type"] || "image/jpeg" };
}
