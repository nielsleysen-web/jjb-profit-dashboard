// pages/api/design-tasks.js
// Marketing Creatives — Graphic Designer task pipeline.
// Access: admin & Creative Strategist (full), Graphic Designer (status, output links, chat),
// Media Buyer (view, status, chat — for the Ready To Launch worktable),
// Video Editor (view, chat).
...
async function applyStatusChange(task, newStatus, session, mediaBuyers) {
  ...
  if (newStatus === "QA Check") {
    ...
    if (session.email !== ADMIN_EMAIL) {
      notifications.push({ email: ADMIN_EMAIL, text: `Design for "${productName}" is ready for QA Check` });
    }
  ...
  if (newStatus === "Ready to launch") {
    for (const mb of mediaBuyers) {
      if (mb.email !== session.email) {
        notifications.push({ email: mb.email, text: `Design for "${productName}" is Ready to launch`, href: "/ready-to-launch" });
      }
    }
  }
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const canStatus = isAdmin || isCS || isGD || isMB;
    ...
    const FIELDS = [
      ...
      "batchType", "visualBriefing", "iterationType", "creativeCopy",
    ...
    if (action === "chat") {
      ...
      for (const u of activeUsers) {
        ...
        if (first && new RegExp(`@${first.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}\\b`, "i").test(text)) {
          notifs.push({ email: u.email, text: `${session.name} mentioned you on "${task.product?.title || "a design task"}": "${text.slice(0, 80)}"` });
        }
      }
      await writeData("design-tasks", store);
      await pushNotifications(notifs);
    ...
    if (action === "duplicate") {
      ...
      addLog(copy, session, `duplicated this task from "${task.product?.title || "a design task"}"`);
    ...
    console.error("Design tasks error:", error.message);
